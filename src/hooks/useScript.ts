...
export default function useScript() {
  ...
  const WEDDING_DATES: Record<number, keyof typeof EVENT_CONFIG> = {
    7: 'Ijab & Qabul', // Night
    8: 'Sanding', // Grandeur
    14: 'Tandang', // Journey
  };
  ...
  let currentEventTag: keyof typeof EVENT_CONFIG | null = null;
  let selectedFile: File | null = null;
  let globalMaxPhotoId = 0;

  // ===== UTILITY FUNCTIONS =====
  function applyTheme(theme: typeof currentEventTag) {
  ...
  async function unapprovePhoto(photoId: number) {
    try {
  ...
  async function extractPhotoTimestamp(file: File) {
    try {
  ...
  function validatePhotoDate(takenAtISO: string) {
  ...
  function showModerationRejection(message: string) {
  ...
  function showTestModeSelector(): Promise<{
    eventTag: keyof typeof EVENT_CONFIG;
    label?: string;
  } | null> {
    return new Promise((resolve) => {
      ...
      const closePopup = (
        result: { eventTag: keyof typeof EVENT_CONFIG; label?: string } | null,
      ) => {
        overlay.classList.remove('visible');
        setTimeout(() => overlay.remove(), 300);
        resolve(result);
      ...
      overlay.querySelectorAll('.test-option').forEach((btn) => {
        const buttonTyped = btn as HTMLButtonElement;
        buttonTyped.addEventListener('click', () => {
          closePopup({
            eventTag: buttonTyped.dataset.event as keyof typeof EVENT_CONFIG,
            label: buttonTyped.dataset.label,
          });
        });
      });

      (
        overlay.querySelector('.test-cancel') as HTMLButtonElement
  ...
  async function loadPhotosForEvent(
    ...
    const gallery = document.getElementById(config.gallery) as HTMLDivElement;
    ...
    try {
      ...
      if (photos.length > 0) {
        const batchMax = Math.max(
          ...photos.map((p: PhotoResponse['photos'][number]) => p.id),
        );
  ...
  function createPhotoCard(
    ...
    card.innerHTML = `
        <div class="photo-item" role="listitem">
            <img src="${photo.url}" alt="Wish from ${photo.name || 'Guest'}${photo.message ? ': ' + photo.message : ''}" loading="lazy">
            ${isPending ? `<div class="reviewing-badge" aria-label="This photo is currently being reviewed">Reviewing photo...</div>` : ''}
            ${false ? `<button onclick="unapprovePhoto(${photo.id})" class="unapprove-btn" title="Remove from guestbook" aria-label="Remove wish by ${photo.name || 'Guest'} from guestbook">✕</button>` : ''}
            ${hasEditToken ? `<button class="edit-btn" data-photo-id="${photo.id}" data-photo-url="${photo.url}" data-photo-name="${(photo.name || '').replace(/"/g, '&quot;')}" data-photo-message="${(photo.message || '').replace(/"/g, '&quot;')}" data-event-tag="${eventTag || photo.eventTag || ''}" title="Edit your submission" aria-label="Edit your photo submission">Edit</button>` : ''}
        </div>
    ...
    if (hasEditToken) {
      ...
      if (editBtn) {
        editBtn.addEventListener('click', () => {
          const photoId = parseInt(editBtn.dataset.photoId as string);
          ...
          openEditModal(
            ...
            photoName,
            photoMessage,
            eventTag as keyof typeof EVENT_CONFIG,
          );
        });
      }
    }
  ...
  function createLoadMoreCard(eventTag: keyof typeof EVENT_CONFIG) {
    const card = document.createElement('div');
  ...
  function appendPhotos(
    ...
    const existingLoadMore = gallery.querySelector(
      '.load-more-card',
    ) as HTMLDivElement;
    if (existingLoadMore) {
  ...
  function setupPhotoEntranceObserver(
    gallery: HTMLDivElement,
    specificCards: HTMLDivElement[] | null = null,
  ) {
    const photoCards = (specificCards ||
      gallery.querySelectorAll(
        '.photo-card:not(.visible)',
      )) as HTMLDivElement[];
  ...
  async function verifyLocation(): Promise<{
    success: boolean;
    reason: string;
  }> {
    return new Promise((resolve) => {
  ...
  async function uploadPhoto(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          response.status === 400 &&
          result.code === 'TEXT_MODERATION_FAILED'
        ) {
      ...
      (document.getElementById('photoForm') as HTMLFormElement).reset();
      (document.getElementById('photoName') as HTMLInputElement).value =
        savedName;
      (document.getElementById('uploadPreview') as HTMLDivElement).innerHTML = `
  ...
  let currentEditPhotoId: number | null = null;
  let currentEditEventTag: keyof typeof EVENT_CONFIG | null = null;

  async function editPhoto(photoId: number, name: string, message: string) {
    const editTokens = JSON.parse(
      localStorage.getItem(EDIT_TOKENS_KEY) || '{}',
    );
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (updatedCard) {
          ...
          const photoName = updatedCard.querySelector(
            '.photo-name',
          ) as HTMLParagraphElement;
          const photoMessage = updatedCard.querySelector(
            '.photo-message',
          ) as HTMLParagraphElement;
          const editBtn = updatedCard.querySelector(
            '.edit-btn',
          ) as HTMLButtonElement;
  ...
  function openEditModal(
    ...
    (document.getElementById('editPhotoName') as HTMLInputElement).value =
      name || '';
    (document.getElementById('editPhotoMessage') as HTMLInputElement).value =
      message || '';
    (
      document.getElementById('editEventIndicator') as HTMLSpanElement
    ).textContent = config?.label || eventTag || '';
  ...
  // ===== MODAL CONTROL =====
  function openModal(eventTag: keyof typeof EVENT_CONFIG) {
    const config = EVENT_CONFIG[eventTag];
    if (!config) return;

    currentEventTag = eventTag;
    const modal = document.getElementById('uploadModal') as HTMLDivElement;
    ...
    (document.getElementById('hiddenEventTag') as HTMLInputElement).value =
      eventTag;
    (document.getElementById('eventIndicator') as HTMLSpanElement).textContent =
      config.label;
  ...
  function setupScrollObserver() {
    ...
    const navbar = document.querySelector('.floating-nav') as HTMLDivElement;
    ...
    function updateActiveSection() {
      ...
      sectionTitles.forEach((title) => {
        const section = title.closest('.gallery-section') as HTMLDivElement;
      ...
      if (!activeSection && sections.length > 0) {
        activeSection = sections[0] as HTMLDivElement;
      }

      if (activeSection) {
        const theme = activeSection.dataset.theme as keyof typeof EVENT_CONFIG;
        const event = activeSection.dataset.event as keyof typeof EVENT_CONFIG;
  ...
  function setupUploadButton() {
    const uploadCta = document.getElementById('uploadCta') as HTMLButtonElement;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    Object.keys(EVENT_CONFIG).forEach((eventTag) =>
      loadPhotosForEvent(eventTag as keyof typeof EVENT_CONFIG, false),
    );
    ...
    // Navigation clicks
    (
      document.querySelectorAll('.nav-item') as NodeListOf<HTMLButtonElement>
    ).forEach((item) => {
      item.addEventListener('click', () => {
        const sectionId = item.dataset.section as string;
        (document.getElementById(sectionId) as HTMLDivElement).scrollIntoView({
          behavior: 'smooth',
        });
      });
    });
    ...
    (
      document.getElementById('uploadCta') as HTMLButtonElement
    ).addEventListener('click', async () => {
      if (await validateAccess()) {
        (
          document.getElementById('hiddenFileInput') as HTMLInputElement
        ).click();
      }
    });
    ...
    (
      ...
      const takenAt = await extractPhotoTimestamp(file);
      let eventTag: keyof typeof EVENT_CONFIG;
      ...
      if (isTestMode) {
        ...
        eventTag = validation.eventTag as keyof typeof EVENT_CONFIG;
      }
      ...
      (document.getElementById('photoFile') as HTMLInputElement).files =
        dataTransfer.files;
      selectedFile = file;
    });

    // Upload zone click
    (document.getElementById('uploadZone') as HTMLDivElement).addEventListener(
      'click',
      () => {
        (document.getElementById('photoFile') as HTMLInputElement).click();
      },
    );

    // Photo file change in modal
    (document.getElementById('photoFile') as HTMLInputElement).addEventListener(
      'change',
      (e) => {
        const file = (e.target as HTMLInputElement).files?.[0];
        if (file) {
    ...
    // Form submission
    (document.getElementById('photoForm') as HTMLFormElement).addEventListener(
      'submit',
      async (e) => {
        e.preventDefault();
        const file =
          selectedFile ||
          (document.getElementById('photoFile') as HTMLInputElement).files?.[0];
        const name = (
          document.getElementById('photoName') as HTMLInputElement
        ).value.trim();
        const message = (
        ...
        ).value as keyof typeof EVENT_CONFIG;
    ...
    (
      document.getElementById('modalBackdrop') as HTMLDivElement
    ...
    // Edit modal handlers
    (
      document.getElementById('editModalBackdrop') as HTMLDivElement
    ).addEventListener('click', closeEditModal);
    (
      document.getElementById('editModalClose') as HTMLButtonElement
    ).addEventListener('click', closeEditModal);
    ...
    (document.getElementById('editForm') as HTMLFormElement).addEventListener(
      ...
      async (e) => {
        e.preventDefault();
        const name = (
          document.getElementById('editPhotoName') as HTMLInputElement
        ).value.trim();
        const message = (
          document.getElementById('editPhotoMessage') as HTMLInputElement
        ).value.trim();
    ...
    (
      document.getElementById('editDeleteBtn') as HTMLButtonElement
    ).addEventListener('click', async () => {
      if (currentEditPhotoId) {
  ...
  async function pollForNewPhotos() {
    ...
    try {
      const pendingIds = Array.from(
        document.querySelectorAll('.reviewing-badge'),
      )
        .map(
          (badge) =>
            (badge.closest('.photo-card') as HTMLDivElement)?.dataset.photoId,
        )
        .filter((id) => id);
      ...
      const newPhotos = photos.filter(
        (p: PhotoResponse['photos'][number]) => p.id > globalMaxPhotoId,
      );
      const batchMax = Math.max(
        ...newPhotos.map((p: PhotoResponse['photos'][number]) => p.id),
      );
      if (batchMax > globalMaxPhotoId) globalMaxPhotoId = batchMax;

      // Distribute photos to their respective galleries (Reverse to maintain order when prepending)
      photos.reverse().forEach((photo: PhotoResponse['photos'][number]) => {
        const config =
          EVENT_CONFIG[photo.eventTag as keyof typeof EVENT_CONFIG];
        if (!config) return;
